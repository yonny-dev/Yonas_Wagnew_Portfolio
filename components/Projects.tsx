...
import { motion } from 'framer-motion'; // Import motion for scroll-triggered animations
import { Github, ExternalLink, ArrowRight } from 'lucide-react'; // Import icons from lucide-react
  ...
  return (
    ...
    <section id="projects" className="py-32 scroll-mt-32">
      ...
      <div className="flex items-center space-x-6 mb-8">
        <span className="h-[2px] w-12 bg-[#DA121A]"></span> {/* Red accent line */}
        <span className="text-[#DA121A] font-bold uppercase tracking-[0.3em] text-[10px]">Archive</span>
      </div>
      
      {/* Section header with title and "Explore All" link */}
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-20">
        ...
        {/* Link to external GitHub profile */}
        <a 
      ...
      {/* List of projects with alternating layouts */}
      <div className="space-y-32">
        {PROJECTS.map((project, idx) => (
          // Individual project container with entrance animation
          <motion.div
            key={project.id}
            ...
            // Alternating layout logic for visual rhythm with responsive gap
            className={`grid grid-cols-1 lg:grid-cols-12 gap-10 md:gap-16 lg:gap-20 items-center ${idx % 2 === 0 ? '' : 'lg:flex-row-reverse'}`}
          >
            {/* Project Image Column */}
            <div className={`lg:col-span-7 ${idx % 2 === 0 ? '' : 'lg:order-2'}`}>
              <div className="group relative rounded-[24px] md:rounded-[40px] overflow-hidden glass-card aspect-[16/9] shadow-2xl">
                {/* Project screenshot with hover zoom and grayscale transition */}
                <img 
                  src={project.image} 
                  alt={project.title} 
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-10">
                  <div className="flex space-x-4">
                    ...
                    <a href={project.githubUrl} className="p-4 bg-white text-black rounded-2xl hover:bg-[#FCDC04] transition-colors shadow-2xl" target="_blank" rel="noopener noreferrer">
                      <Github size={24} />
                    </a>
                    {/* Live platform link */}
                    <a href={project.liveUrl} className="p-4 bg-white text-black rounded-2xl hover:bg-[#FCDC04] transition-colors shadow-2xl" target="_blank" rel="noopener noreferrer">
            ...
            <div className={`lg:col-span-5 ${idx % 2 === 0 ? '' : 'lg:order-1'}`}>
              ...
              {/* Primary action link */}
              <a href={project.liveUrl} target="_blank" rel="noopener noreferrer" className="text-zinc-900 dark:text-white font-bold text-[10px] uppercase tracking-[0.3em] inline-flex items-center space-x-3 group">
